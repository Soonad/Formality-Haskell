    ...
    synDecl d = case d of
      ...
      Lang.Enum n ns -> case n of
        Just n -> do
          ...
          let enumTyp   = (n, Ann Typ (App (Ref "Enum" 0) str Eras))
          let enumVal x = App (App (Ref "enum" 0) str Keep) (U64 x) Keep
...
buildString txt = let
  ...
  t     = App (Ref "nil" 0) Wrd Eras
  f a b = App (App (App (Ref "cons" 0) Wrd Eras) (U64 a) Keep) b Keep
  in foldr f t nums
